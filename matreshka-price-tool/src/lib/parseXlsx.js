...
export async function parsePriceXlsx(file) {
  ...
  // anchors use absolute 0-based rows — add this offset or every image is
  // mismatched by the start offset.
...
function assignImages(products, anchors) {
  ...
  // Pass 1 — exact from-row, locked first so nothing can steal it. Kills the
  // cascade where a volume variant without its own anchor grabs the next row's
  // photo and shifts every image down by one.
  ...
  // Pass 2 — span / nearest among leftovers, only for rows still unmatched.
  const pick = (rowIdx) => {
